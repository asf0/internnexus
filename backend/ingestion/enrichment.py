...
def normalize_location(location: str) -> dict:
    ...
    if (re.match(r'^\d+', location) and 
        any(keyword in location_lower for keyword in ['street', 'avenue', 'ave', 'st', 'rd', 'dr', 'ln', 'blvd', 'pkwy', 'plaza', 'road'])):
        return {"full": None, "city": None, "state": None, "country": None}
    ...
    has_country_at_end = (last_part and 
                          len(last_part) <= 3 and 
                          last_part in COUNTRIES and
                          last_part not in US_STATES)  # Exclude if it's also a state
...
class LegendAttributeDetector:
    ...
    def detect_is_faang_plus(self, company: str) -> bool:
        ...
        faang_companies = {
            ...
            "microsoft", "tesla", "nvidia", "stripe", "databricks", "figma",
            "discord", "airbnb", "dropbox", "twilio", "okta", "plaid",
            "cloudflare", "coinbase", "roblox"
        }
...
class CategoryDetector:
    ...
    def detect_category(self, title: str, description: str) -> str | None:
        ...
        if any(word in title_lower for word in ["software", "engineer", "developer", "programmer", "backend", "frontend", "fullstack", "swe"]):
            return "software_engineering"
        
        # Product Management patterns
        if any(word in title_lower for word in ["product", "pm", "manager"]) and "product engineer" not in title_lower:
            return "product_management"
        
        # Data Science & AI patterns
        if any(word in title_lower for word in ["data", "machine learning", "ml", "ai", "artificial intelligence", "data science", "analytics", "data analyst"]):
            return "data_science_ai"
        
        # Quantitative Finance patterns
        if any(word in title_lower for word in ["quant", "trading", "finance", "financial", "risk analysis", "derivatives"]):
            return "quantitative_finance"
        
        # Hardware Engineering patterns
        if any(word in title_lower for word in ["hardware", "circuit", "embedded", "firmware", "electrical", "mechanical", "physical design"]):
            return "hardware_engineering"
        
        return None


def enrich_jobs(jobs: list[JobSchema], category_context: dict[str, str] | None = None, skip_embedding: bool = False) -> list[JobSchema]:
    ...
    for job in jobs:
        ...
        job.location = location_data.get("full") or job.location  # Keep original if normalization failed
        ...
        if job.description_text:
            ...
            if embedder:
                try:
                    job.description_embedding = embedder.embed(job.description_text)
            ...
            job.requires_sponsorship = legend_detector.detect_requires_sponsorship(job.description_text, job.title)
            job.requires_us_citizenship = legend_detector.detect_requires_us_citizenship(job.description_text, job.title)
            job.application_closed = legend_detector.detect_application_closed(job.description_text, job.title)
            job.requires_advanced_degree = legend_detector.detect_requires_advanced_degree(job.description_text, job.title)
        ...
        if job.company in category_context:
            ...
            job.job_category = category_detector.detect_category(job.title, job.description_text or "")
