...
describe('LoginForm', () => {
  ...
  it('shows loading state when isLoading is true', () => {
    // Arrange
    render(<LoginForm isLoading={true} onSubmit={vi.fn()} submitLabel="Sign in with Email" />);
  ...
  it('shows custom submit label', () => {
    // Arrange
    render(<LoginForm isLoading={false} onSubmit={vi.fn()} submitLabel="Custom Sign In" />);
  ...
  it('auto-focuses first field when autoFocusFirstField is true', () => {
    // Arrange
    render(<LoginForm isLoading={false} onSubmit={vi.fn()} autoFocusFirstField={true} />);

    // Assert
    const emailInput = screen.getByLabelText(/email address/i);
    expect(emailInput.hasAttribute('autofocus') || document.activeElement === emailInput).toBe(
      true
    );
  });
