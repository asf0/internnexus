...
export default function JobDetailPanel({ job, isLoading, onClose }: JobDetailPanelProps) {
  if (!job && !isLoading) {
    return (
      <div className="flex h-full items-center justify-center rounded-2xl border border-slate-200 bg-white p-8 dark:border-slate-700 dark:bg-slate-900">
  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {job.description_text && (
          <div className="mt-6">
            ...
            <div 
              className="mt-2 prose prose-sm prose-slate max-w-none dark:prose-invert text-slate-600 dark:text-slate-400"
      ...
      {job.application_closed && (
        <div className="border-t border-slate-200 p-6 dark:border-slate-700">
          <div className="flex w-full items-center justify-center gap-2 rounded-lg bg-slate-200 px-4 py-3 font-medium text-slate-500 dark:bg-slate-700 dark:text-slate-400">
            Application Closed
          </div>
        </div>
      )}
