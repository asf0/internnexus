  ...
  return (
    <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Filter Jobs</h3>
        {hasActiveFilters && (
          <button
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <select
          ...
          className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-slate-400 dark:focus:ring-slate-400"
        >
        ...
        <div className="flex gap-2">
          <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:hover:bg-slate-700">
            <input
              type="checkbox"
              checked={currentVisa === "true"}
              onChange={(e) => updateFilter("visa_sponsored", e.target.checked ? "true" : "")}
              className="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
            />
            <span className="text-slate-700 dark:text-slate-300">Visa</span>
          </label>
          <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:hover:bg-slate-700">
            <input
              type="checkbox"
              ...
              className="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
            />
            <span className="text-slate-700 dark:text-slate-300">F1</span>
          </label>
        </div>
      </div>

      {isPending && (
        <div className="text-xs text-slate-500 dark:text-slate-400">Updating results...</div>
      )}
    </div>
  );
}
