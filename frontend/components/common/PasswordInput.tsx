...
function calculateStrength(password: string): PasswordStrength {
  const requirements: Requirement[] = [
    ...
    { label: '1 number', met: /[0-9]/.test(password) },
    {
      ...
      met: new RegExp(`[${SPECIAL_CHARS.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$')}]`).test(
        password
      ),
    },
  ];
...
function getStrengthTextColor(level: PasswordStrength['level']): string {
  switch (level) {
    ...
    case 'good':
      return 'text-md-primary dark:text-md-primary';
    case 'strong':
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            disabled={disabled}
            className="dark:border-md-outline-variant focus:ring-md-primary focus:border-md-primary dark:bg-md-surface-container dark:text-md-on-surface block w-full rounded-lg border border-slate-300 px-3 py-2 pr-10 placeholder-slate-400 shadow-sm focus:ring-2 focus:outline-none disabled:opacity-50"
        ...
        {showStrengthIndicator && value.length > 0 && (
          <div className="mt-2">
            <div className="mb-1 flex items-center justify-between">
              ...
              <span
                className={`text-xs font-medium capitalize ${getStrengthTextColor(strength.level)}`}
              >
                {strength.level}
              </span>
            </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              disabled={disabled}
              className={`focus:ring-md-primary focus:border-md-primary dark:bg-md-surface-container dark:text-md-on-surface block w-full rounded-lg border px-3 py-2 pr-10 placeholder-slate-400 shadow-sm focus:ring-2 focus:outline-none disabled:opacity-50 ${
