...
export default function MultiSelect({ options, selected, onChange, placeholder, labelMap }: MultiSelectProps) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        {selected.length === 0 ? (
          <span className="text-slate-400 dark:text-md-on-surface-variant">{placeholder}</span>
        ) : (
          selected.map((item) => (
            <span
              key={item}
            ...
            >
              ...
              <button
                ...
                className="hover:text-slate-900 dark:hover:text-md-on-surface"
              >
                <X size={12} />
              </button>
            </span>
        ...
        <div className="absolute z-10 mt-1 w-full rounded-lg border border-slate-300 bg-white shadow-lg dark:border-md-outline-variant dark:bg-md-surface-container">
          <div className="p-2">
            <input
              ...
              className="w-full rounded border border-slate-300 bg-white px-3 py-1.5 text-sm text-slate-900 focus:border-slate-500 focus:outline-none dark:border-md-outline-variant dark:bg-md-surface-container-high dark:text-md-on-surface"
