...
export default function MatchedJobList({ totalPages: _totalPages, currentPage }: MatchedJobListProps) {
    ...
    return (
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold dark:text-slate-100">Matched roles</h2>
        </div>
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold dark:text-slate-100">Matched roles</h2>
        <span className="text-sm text-slate-500 dark:text-slate-400">{total} matches</span>
      </div>
        ...
        <div className={`transition-all duration-300 ${selectedJob ? "w-full lg:w-1/2" : "w-full"}`}>
          ...
          {jobs.length === 0 && (
            <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
              <p className="text-slate-500 dark:text-slate-400">No matched jobs found.</p>
            </div>
          )}
