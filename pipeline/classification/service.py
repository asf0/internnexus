...
def _extract_batch_categories(raw_output: str, expected_count: int) -> list[tuple[str | None, str, str]]:
...
class JobClassifier:
    ...
    def __init__(
        ...
        self._batch_size = int(getattr(settings, "classification_batch_size", 10))
    ...
    async def _classify_prompt_batch_with_reasons(
        self,
    ...
    async def _classify_lmstudio_batch(self, jobs: list[tuple[str, str]]) -> list[tuple[str | None, str, str]]:
        return await self._classify_lmstudio_batch_impl(jobs)

    @_retry_decorator
    async def _classify_lmstudio_batch_impl(self, jobs: list[tuple[str, str]]) -> list[tuple[str | None, str, str]]:
        system_prompt, user_prompt = _build_batch_classification_prompts(jobs)
        client = await self._get_client()
    ...
    async def _classify_batch_individually_with_reasons(
        self,
        jobs: list[tuple[str, str]],
    ) -> list[tuple[str | None, str, str]]:
        results: list[tuple[str | None, str, str]] = []
    ...
    async def classify_batch_with_reasons(self, jobs: list[tuple[str, str]]) -> list[tuple[str | None, str]]:
        ...
        batch_size = max(1, min(self._batch_size, len(jobs)))
            ...
            async with semaphore:
                try:
                    if len(chunk) == 1 or batch_size == 1:
                        ...
                        chunk_results = await self._classify_prompt_batch_with_reasons(chunk)
                        if any(reason in {"invalid_json", "invalid_json_shape"} for _category, reason, _raw in chunk_results):
                            logger.warning(
